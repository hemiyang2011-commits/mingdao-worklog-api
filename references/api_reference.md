...
- **部门表 / 工作任务表未授权**：工作日志的「部门」（`6aa23c69683cab0ff217d1b8`）和
  「工作任务」（`699fc3c07269639034797d28`）是关联字段，指向部门表
  （`69377f4a5326c71216b47c3a`）和工作任务表（`699fb4a72216b8b595a942ef`）。这两张表
  未加入应用授权，v2 getWorksheetInfo / getFilterRows 与 v3 读行都报 `10005`，
  **无法按名称反查 rowid**。写日志时只能直接传 rowid；rowid 可从已填该字段的
  日志行 getFilterRows 反查（关联项里有 `sid` + `name`）。本组织实测：
  部门「研发部」= `7182133b-6e95-460b-8bb8-55fe05c9905d`，
  工作任务「开发」= `5f55a6f8-11a8-4f2a-a178-c467bdb3820f`（全表仅此一条叫「开发」的共享任务）。
